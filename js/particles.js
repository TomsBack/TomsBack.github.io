...
var Particles = (function(window, document) {
  ...
  Plugin.prototype._drawEdge = function(p1, p2, opacity) {
    let p1IsRgb = p1.color.includes("rgb");
    let p2IsRgb = p2.color.includes("rgb");

    var _ = this,
        gradient = _.context.createLinearGradient(p1.x, p1.y, p2.x, p2.y);

    var color1 = p1IsRgb ? this._rgbStrToRgb(p1.color) : this._hex2rgb(p1.color);
    var color2 = p2IsRgb ? this._rgbStrToRgb(p2.color) : this._hex2rgb(p2.color);
  ...
  /**
   * Converts a rgb string to a rgb object.
   *
   * @private
   * @param {string} rgb
   * @return {object}
  ...
  Plugin.prototype._rgbStrToRgb = function(rgb) {
    let result = rgb.replaceAll("rgb(", "").replaceAll(")", "").replaceAll(" ", "").split(",");
